...
function registerSW(){
  ...
  navigator.serviceWorker.register('/DesafioHV3/service-worker.js').then(reg=>{
...
// ══════════════════════════════════════════════════════════
const MISSION_POOL=[
  {id:'all_done',icon:'🏆',name:'Día perfecto',desc:'Completá el 100% de tus actividades (mín. 3).',stars:2,check:()=>calcTodayPct()===100&&activities.length>=MIN_ACTS_FOR_PERFECT},
  {id:'no_skip',icon:'🎯',name:'Sin excusas',desc:'100% del día sin ningún "No hice" (mín. 3 actividades).',stars:3,check:()=>activities.length>=MIN_ACTS_FOR_PERFECT&&activities.every(a=>!isSkippedAct(a.id))&&calcTodayPct()===100},
  ...
  {id:'hard_perfect',icon:'☠️',name:'Sin compasión',desc:'Día al 100% en Modo Difícil.',stars:4,check:()=>getMode()==='hard'&&calcTodayPct()===100&&activities.length>=MIN_ACTS_FOR_PERFECT},
  {id:'medium_perfect',icon:'⚡',name:'Precisión media',desc:'Día al 100% en Modo Media.',stars:2,check:()=>getMode()==='medium'&&calcTodayPct()===100&&activities.length>=MIN_ACTS_FOR_PERFECT},
...
const ACHIEVEMENTS=[
  ...
  {id:'first_day',icon:'📅',name:'Primer día',desc:'Completá un día al 100% (mín. 3 actividades).',cat:'Inicio',stars:1,check:()=>(calcTodayPct()===100&&activities.length>=MIN_ACTS_FOR_PERFECT)||calcTotalPerfect()>=1},
  ...
  {id:'no_skip_day',icon:'🎖️',name:'Día impoluto',desc:'Día al 100% sin ningún "No hice" (mín. 3 actividades).',cat:'Perfección',stars:2,check:()=>activities.length>=MIN_ACTS_FOR_PERFECT&&activities.every(a=>!isSkippedAct(a.id))&&calcTodayPct()===100},
  ...
  {id:'mode_medium_first',icon:'🟡',name:'Modo Media',desc:'Completá un día en Modo Media.',cat:'Modos',stars:2,check:()=>getMode()==='medium'&&calcTodayPct()===100&&activities.length>=MIN_ACTS_FOR_PERFECT},
  ...
  {id:'mode_hard_perfect',icon:'💀',name:'Sin piedad',desc:'Día al 100% en Modo Difícil.',cat:'Modos',stars:4,check:()=>getMode()==='hard'&&calcTodayPct()===100&&activities.length>=MIN_ACTS_FOR_PERFECT},
...
function calcTotalPerfect(){return getHistory().filter(h=>!h.skipped&&calcSnapPct(h)===100&&h.total>=MIN_ACTS_FOR_PERFECT).length;}
...
function confirmActDone(id){
  ...
  const pct=calcTodayPct();if(pct===100&&activities.length>=MIN_ACTS_FOR_PERFECT){setTimeout(()=>{launchConfetti();showToast('🏆 ¡100% del día!');},700);}
}
...
window.openFriendProfile = async function(uid, username) {
  ...
  try {
    ...
    const modeLabels = { easy: '🟣 Fácil', medium: '🟡 Media', hard: '🔴 Difícil' };
    body.innerHTML = `
      <div class="fp-note">🔒 Por privacidad, solo se muestran métricas generales — nunca el detalle de sus actividades.</div>
      <div class="fp-grid">
